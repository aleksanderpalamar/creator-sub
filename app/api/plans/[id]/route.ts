...
export async function PUT(
  ...
  try {
    ...
    if (!plan) {
      return NextResponse.json(
        { message: "Assinatura nao encontrado" },
        { status: 404 }
      );
    }
    ...
    if (plan.creatorId !== session.user.id) {
      return NextResponse.json(
        { message: "Você não tem permissão para editar essa assinatura" },
        { status: 403 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (plan.creatorId !== session.user.id) {
      return NextResponse.json(
        { message: "Voce nao tem permissao para deletar essa assinatura" },
        { status: 403 }
      );
    }
