    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">{plans.length} assinatura(s) disponível(is)</p>
        </div>
      </div>
      ...
      {existingSubscription && (
        <Card className="bg-primary/10">
          <CardHeader>
            ...
            <CardDescription>Você já assina o plano {existingSubscription.subscriptionPlan.name}</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm">Valor: {formatCurrency(existingSubscription.subscriptionPlan.price)}/mês</p>
          </CardContent>
          <CardFooter>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {plans.map((plan) => (
          <Card key={plan.id} className={existingSubscription?.subscriptionPlanId === plan.id ? "border-primary" : ""}>
            <CardHeader>
            ...
            <CardFooter>
              {existingSubscription?.subscriptionPlanId === plan.id ? (
                ...
                <SubscribeButton creatorId={creator.id} planId={plan.id} disabled={!!existingSubscription} />
              )}
            </CardFooter>
          </Card>
        ))}
